		...
		var Clones = {
			...
			collect: function collect() {
				...
				for (var i = 0; i < pattern.length; i++) {
					var clone = Components.Html.slides[Math.abs(pattern[i])].cloneNode(
						true
					);
			...
			append: function append() {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (pattern[i][0] === '-') {
						Components.Html.wrapper.insertBefore(
							item,
		...
		var Swipe = {
			...
			bindSwipeMove: function bindSwipeMove() {
				Binder.on(
					MOVE_EVENTS,
					Components.Html.wrapper,
					throttle(this.move.bind(this), Glide.settings.throttle)
				);
			},
	...
	var Controls = function(Glide, Components, Events) {
		...
		var Controls = {
			...
			click: function click(event) {
				...
				Components.Run.make(
					Components.Direction.resolve(event.currentTarget.dataset.glideDir)
				);
			}
		};
	...
	var Glide = (function(_Core) {
		...
		function Glide() {
			...
			return possibleConstructorReturn(
				this,
				(Glide.__proto__ || Object.getPrototypeOf(Glide)).apply(this, arguments)
			);
		}
		...
		createClass(Glide, [
			{
				...
				value: function mount() {
					...
					return get(
						Glide.prototype.__proto__ || Object.getPrototypeOf(Glide.prototype),
						'mount',
						this
